...
UdpClient::UdpClient(const std::string& _ip, int _port) :fd_socket_(INVALID_SOCKET) ,
    event_(NULL) , selector_(breaker_, true) , thread_(NULL) {
    __InitSocket(_ip, _port);
}

UdpClient::UdpClient(const std::string& _ip, int _port, IAsyncUdpClientEvent* _event) :fd_socket_(INVALID_SOCKET) ,
    event_(_event) , selector_(breaker_, true) {
...
bool UdpClient::HasBufferToSend() {
