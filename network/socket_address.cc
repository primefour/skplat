...
/*
        ...
        skdebug("ret =%d, ip_=%s, nat64_v6_addr = %s", ret, ip_, strutil::Hex2Str((char*)&(nat64_v6_addr.s6_addr32), 16).c_str());
        if (ret) {
            ...
            //-----------------------------//
            snprintf(url_, sizeof(url_), "[%s]:%u", ip_, port());
            skdebug("after fix url_=%s", url_);
        } else {
            skerror("ConvertV4toNat64V6() ret=%d, ipstack=%s", ret, TLocalIPStackStr[local_ipstack_detect()]);
        }

        */
    }
	skdebug("is_update =%d, ret=%d", is_update, ret);
	return ret;
}
const sockaddr& socket_address::address_fix() {
	if (AF_INET6 == addr_.sa.sa_family) {
		skdebug("before fix current ipv6 = %s", ipv6());
		fix_current_nat64_addr();
		skdebug("after fix current ipv6 = %s", ipv6());
	}
...
bool socket_address::valid_server_address(bool _allowloopback) const {
    if (AF_INET == addr_.sa.sa_family) {
        ...
        if (IN6_IS_ADDR_V4MAPPED(&(sock_addr6.sin6_addr))) {
            uint32_t temp = 0;
            memcpy(&temp, &sock_addr6.sin6_addr.s6_addr[12], sizeof(uint32_t));
            uint32_t hostip = ntohl(temp);
