...
jmethodID VarCache::GetStaticMethodId(JNIEnv* _env, jclass _clz,
    ...
    //__android_log_print(ANDROID_LOG_ERROR, "debug", "method name %s  signature %s ",_method_name,_signature);
...
bool LoadStaticMethod(JNIEnv* _env) {
    ...
    for (std::set<JniMethodInfo>::iterator iter = methoNameSet.begin(); iter != methoNameSet.end(); ++iter) {
        ...
        //__android_log_print(ANDROID_LOG_ERROR,"debug" ,"%s %d  class name %s ==> method name %s sig==> %s",__func__,__LINE__,
        //       (*iter).classname.c_str(),(*iter).methodname.c_str(),(*iter).methodsig.c_str());
