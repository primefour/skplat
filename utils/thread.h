...
class Thread {
    ...
    public:
        template<class T>
        explicit Thread(const T& op, const char* _thread_name = NULL): runable_ref_(NULL) {
            runable_ref_ = new RunnableReference(transform(op));
        ...
        Thread(const char* _thread_name = NULL): runable_ref_(NULL) {
        ...
        int start(const T& op, bool* _newone = NULL) {
            ...
            runable_ref_->target = transform(op);
