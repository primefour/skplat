...
static void __register_anr(uintptr_t _ptr, const char* _file, const char* _func, int _line, int _timeout) {
    ...
    check_content ch = {_ptr, _file, _func, _line, _timeout,GetTid(), clock_app_monotonic(), 0, gettickcount(), 0/*init cpu_time*/};
...
static void __anr_checker_thread() {
    while (true) {
        ...
        if (sg_check_heap.empty()) {
            ...
            if (wait_timeout<0) {
                skwarn("%p wait_timeout:%" PRId64 ", end_time:%" PRId64 ", used_cpu_time:%" PRId64 ", now:% " PRId64 ", anr_checker_size:%zd",(void*)(sg_check_heap.front().ptr), wait_timeout,
        ...
        if (is_wait_timeout && round_tick_elapse > (wait_timeout+kTimeDeviation)) {
            skwarn("%p now:%" PRId64 ", round_tick_start:%" PRId64 ", round_tick_elapse:%" PRId64 ", wait_timeout:%" PRId64 ", round cputime:%" PRId64 ", anr_checker_size:%zd", 
                    ((void*)sg_check_heap.front().ptr),clock_app_monotonic(), round_tick_start, round_tick_elapse, wait_timeout, use_cpu_time_2-use_cpu_time_1, sg_check_heap.size());
            iOS_style = true;
        }
        ...
        if (iOS_style) {
            if (!sg_check_heap.empty() && (uint64_t)sg_check_heap.front().timeout <= sg_check_heap.front().used_cpu_time) {
                check_hit = true;
                skassert2(sg_check_heap.front().end_time <= clock_app_monotonic(),
                         "end_time:%" PRIu64", now:%" PRIu64", anr_checker_size:%d, @%p", 
                         sg_check_heap.front().end_time, clock_app_monotonic(), 
                         (int)sg_check_heap.size(), (void*)sg_check_heap.front().ptr); //old logic is strict than new logic
            }
        } else {
