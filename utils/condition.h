  ...
  public:
    ...
    void wait(Mutex &mutex){
        ...
        if (!atomic_cas32(&anyway_notify_, 0, 1)) {
            ret = pthread_cond_wait(&condition_, &(mutex.internal()));
        }
        anyway_notify_ = 0;
    ...
    void wait(Mutex &mutex,int64_t millisecond){
        struct timespec ts;
        makeTimeout(&ts, (long)millisecond);
        int ret = 0;
        if (!atomic_cas32(&anyway_notify_, 0, 1)) {
            ret = pthread_cond_timedwait(&condition_, &(mutex.internal()), &ts);
        }

        anyway_notify_ = 0;

        if (ETIMEDOUT == ret || 0 == ret) return ;
