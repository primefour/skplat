...
int insert_dns(SQLite *sqlite,network_dns *entry){
    ...
    snprintf(sql_buff,sizeof(sql_buff),"insert into dns(host,ip,dns_server,ip_type,dns_type,fail_times,conn_profile) values (\'%s\',\'%s\',\'%s\',%d,%d,%d,%ld);",
...
int update_dns_connprofile(SQLite *sqlite,const char *host,const char *ip,int64_t conn_profile){
    ...
    snprintf(sql_buff,sizeof(sql_buff),"update dns set conn_profile = %ld where host = '%s' and ip = '%s' ;",conn_profile,host,ip);
    char *zErr = NULL;
    int rc = sqlite3_exec(sqlite->db,sql_buff,NULL,NULL, &zErr);
    if(rc != SQLITE_OK){
        skerror("update %s fail %s ",sql_buff,zErr);
